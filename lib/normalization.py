...
class MMScaler:
    ...
    def transform(self, data):
        return 100 * (data - self.min) / (self.max - self.min)

    def inverse_transform(self, data):
        if type(data) == torch.Tensor and type(self.min) == np.ndarray:
            self.min = torch.from_numpy(self.min).to(data.device).type(data.dtype)
            self.max = torch.from_numpy(self.max).to(data.device).type(data.dtype)
        return (data/100 * (self.max - self.min) + self.min)
...
if __name__ == '__main__':
    ...
    # print(maximum.reshape(3,1))
    # print(test_data-maximum.reshape(3,1))
    # test_data = (test_data-minimum.reshape(3,1)) / (maximum-minimum).reshape(3,1)
    # print(test_data)
    # print(0 == 0)
    # print(0.00 == 0)
    # print(0 == 0.00)
